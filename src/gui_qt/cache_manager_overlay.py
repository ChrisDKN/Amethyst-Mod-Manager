...
class CacheManagerOverlay(OverlayBase):
    ...
    def _on_clear_all(self):
        ...
        body = self.tr("Clear {0} of cached downloads across all entries?"
                ...
                "needed. Cached application tools, runtime installers, curated profiles, images, GitHub responses, Wabbajack gallery data, modlist packages and saved requirement checks are also cleared. The jobs/backups entry removes abandoned jobs and update backups.").format(format_size(total), get_download_cache_dir())
        ConfirmOverlay.show_over(
