...
class TrackedModsPanel:
    """
    Canvas-based panel listing mods tracked by the user on Nexus Mods.
    ...
    def __init__(
        ...
        install_fn : callable
            ``install_fn(entry: TrackedModEntry)`` — called when the user clicks
            Install on a tracked mod.  The parent is responsible for the actual
    ...
    def refresh(self):
        ...
        def _worker():
            try:
                ...
                # Filter to the current game domain
                game_tracked = [
                    t for t in all_tracked
                    if t.get("domain_name", "") == domain
                ]

                entries: list[TrackedModEntry] = []
                total = len(game_tracked)

                for i, t in enumerate(game_tracked):
                    mod_id = t.get("mod_id", 0)
                    if mod_id <= 0:
                        continue
                    entry = TrackedModEntry(
                        mod_id=mod_id,
                        domain_name=domain,
                    )
    ...
    def _on_right_click(self, event):
        ...
        menu.add_command(label="Install Mod",
                         command=lambda e=entry: self._install_fn(e))
        menu.add_separator()
        menu.add_command(label="Untrack Mod",
                         command=lambda e=entry: self._untrack_mod(e))
        menu.tk_popup(event.x_root, event.y_root)

    def _untrack_mod(self, entry: TrackedModEntry):
        """Untrack a mod via the Nexus API."""
        api = self._get_api()
        if api is None:
        ...
        def _worker():
            try:
                ...
                def _done():
                    ...
                    self._status_label.configure(
                        text=f"{count} tracked mod(s) for {entry.domain_name}"
                    )
                self._parent.after(0, _done)
            except Exception as exc:
