"""
Oblivion.py
Game handler for The Elder Scrolls IV: Oblivion.

Mod structure:
  Mods install into <game_path>/Data/
  Staged mods live in Profiles/Oblivion/mods/

Deployment is the same as Fallout 3.
"""
...
class Oblivion(BaseGame):
    ...
    # The Oblivion AppData folder inside the Proton prefix where the game
    ...
    def _plugins_txt_target(self) -> Path | None:
        """Return the in-prefix path where Oblivion expects plugins.txt."""
    ...
    def _swap_launcher(self, log_fn) -> None:
        """Replace OblivionLauncher.exe with obse_loader.exe if present."""
        _log = log_fn
        if self._game_path is None:
            return
        obse = self._game_path / "obse_loader.exe"
        if not obse.is_file():
            _log("  OBSE loader not found — skipping launcher swap.")
            return
        launcher = self._game_path / "OblivionLauncher.exe"
        backup   = self._game_path / "OblivionLauncher.bak"
        if launcher.is_file():
            launcher.rename(backup)
            _log("  Renamed OblivionLauncher.exe → OblivionLauncher.bak.")
        shutil.copy2(obse, launcher)
        _log("  Copied obse_loader.exe → OblivionLauncher.exe.")

    def _restore_launcher(self, log_fn) -> None:
        """Reverse the OBSE launcher swap if a backup exists."""
        _log = log_fn
        if self._game_path is None:
            return
        backup   = self._game_path / "OblivionLauncher.bak"
        ...
        _log("  Restored OblivionLauncher.exe from .bak.")
    ...
    def deploy(self, log_fn=None, mode: LinkMode = LinkMode.HARDLINK,
        ...
        _log("Step 5: Swapping launcher for OBSE ...")
